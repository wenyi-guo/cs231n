...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        loss = -np.sum(np.log(correct_class_score/sum))
        ...
        dev = scores/sum
        dev[range(N), y] -= 1
        grads['W2'] = np.dot(h1.T, dev)/N + reg * W2
        grads['b2'] = np.sum(dev, axis=0)/N
        W2_opposite = np.dot(dev, W2.T)
        W2_opposite_relu = W2_opposite*(h1!=0)
        grads['b1'] = np.sum(W2_opposite_relu, axis=0)/N
        grads['W1'] = np.dot(X.T,W2_opposite_relu)/N + reg * W1
