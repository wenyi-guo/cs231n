...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x, gamma, beta, x_norm, mean, var, eps)
...
def batchnorm_backward(dout, cache):
    ...
    dx_norm = gamma * dout
    dx_2_b = np.sum((x - sample_mean) * dx_norm, axis=0)
    dx_2_a = ((sample_var+ eps) ** -0.5) * dx_norm
    ...
    dx_9_b = np.ones(x.shape) / N * dx_8_b
    dx = dx_9_b + dx_7_a
...
def batchnorm_backward_alt(dout, cache):
    ...
    m = dout.shape[0] 
    dx_norm = dout * gamma
    dvar = (dx_norm * (x-sample_mean) * (-0.5) * np.power(sample_var+eps, -1.5)).sum(axis = 0)  # (D,)
    dmean = np.sum(dx_norm * (-1) * np.power(sample_var + eps, -0.5), axis = 0)
    dmean += dvar * np.sum(-2 * (x - sample_mean), axis = 0) / m
    dx = dx_norm * np.power(sample_var + eps, -0.5) + dvar*2*(x - sample_mean) / m + dmean / m
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    cache = (std, mean, var, x, gamma, beta)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    x_padding = np.pad(x, ((0,), (0,), (pad,), (pad,)), 'constant', constant_values=0)
    out = np.zeros((N, F, H_, W_))
    for i in range(H_):
      for j in range(W_):
        mask = x_padding[:, :, i * stride : i * stride + HH, j * stride : j * stride + WW]
        for k in range(F):
    ...
    for i in range(H_):
        for j in range(W_):
            ...
            for n in range(N): #compute dx_pad
                dx_padding[n, :, i*stride:i*stride+HH, j*stride:j*stride+WW] += np.sum((w[:, :, :, :] *
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_out):
      for j in range(W_out):
        ...
        max_flags = np.max(x_mask, axis=(2, 3), keepdims=True) == x_mask
        dx_mask += max_flags * (dout[:, :, i, j])[:, :, None, None]
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    cache = (std, mean, var, x, gamma, beta, G)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    std, mean, var, x, gamma, beta, G = cache
    N, C, H, W = x.shape
    ...
    dvar_dout_gamma = 2 * np.sum((gamma * dout).reshape(N * G, -1)*(x-mean), axis=1)[...,None]/(x.shape[1])
    dstd_dout_gamma = dvar_dout_gamma / (2*std)
    dx = (((gamma*dout).reshape(N * G, -1) - dmu_dout_gamma) * std - dstd_dout_gamma * (x-mean)) / std**2
