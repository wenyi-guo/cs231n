...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
          input, cache['layer'+str(i+1)] = affine_forward(input, self.params['W'+str(i+1)], self.params['b'+str(i+1)])
          if self.normalization=='batchnorm':
            input, cache['layer'+str(i+1)+'_bn'] = batchnorm_forward(input, self.params['gamma'+str(i+1)], self.params['beta'+str(i+1)], self.bn_params[i])
          if self.normalization=='layernorm':
            input, cache['layer'+str(i+1)+'_ln'] = layernorm_forward(input, self.params['gamma'+str(i+1)], self.params['beta'+str(i+1)], self.bn_params[i])
          input, cache['layer'+str(i+1)+'_relu'] = relu_forward(input)
          if self.use_dropout:
            input, cache['layer'+str(i+1)+'_dropout'] = dropout_forward(input,  self.dropout_param)
        scores, cache['layer'+str(self.num_layers)] = affine_forward(input, self.params['W'+str(self.num_layers)], self.params['b'+str(self.num_layers)])
