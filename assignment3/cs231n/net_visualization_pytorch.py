...
def compute_saliency_maps(X, y, model):
    ...
    saliency, i = torch.max(torch.abs(X.grad), dim=1)
...
def make_fooling_image(X, target_y, model):
    ...
    for i in range(100):
      ...
      if(scores.data.max(dim=1)[1] == target_y):
          break
      else:
          target_scores = scores[:, target_y]
          target_scores.backward()
          grad = X_fooling.grad.data 
          X_fooling.data += learning_rate * grad / torch.norm(grad)
          X_fooling.grad.zero_() # zero image gradient
