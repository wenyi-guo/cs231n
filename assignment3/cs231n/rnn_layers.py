...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h, cache = [], []
    for i in range(T):
      next_h, current_cache = rnn_step_forward(x[:, i, :], prev_h, Wx, Wh, b)
      h.append(next_h)
...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        step_dx, prev_h, step_dWx, step_dWh, step_db = rnn_step_backward(dh[:, t, :] + next_h, cache[t])
        dx.append(step_dx)
        next_h = prev_h
        dWx += step_dWx
        dWh += step_dWh
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, prev_c, prev_h, Wx, Wh, next_c, i, f, o, g)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, prev_c, prev_h, Wx, Wh, next_c, i, f, o, g = cache
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        step_dx, dprev_h, dprev_c, step_dWx, step_dWh, step_db = lstm_step_backward(
                dh[:, t, :] + dnext_h, dnext_c, cache[t])
        dx.append(step_dx)
        dnext_h = dprev_h
        dnext_c = dprev_c
        dWx += step_dWx
